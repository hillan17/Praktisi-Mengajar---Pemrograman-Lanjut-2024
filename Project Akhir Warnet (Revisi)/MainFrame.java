...
public class MainFrame extends JFrame {
    ...
    public MainFrame() {
        ...
        JButton addCustomerButton = new JButton("Choose Computers");
        ...
        JButton removeCustomerButton = new JButton("Finish Rental");
    ...
    private void viewComputers() {
        ...
        if (selectedIndex != -1) {
            String selectedComputer = computerListModel.get(selectedIndex).replace(" (Digunakan)", "");
            Customer customer = customerMap.get(selectedComputer);
            if (customer != null) {
                JOptionPane.showMessageDialog(this, "Komputer sedang digunakan oleh " + customer.getName() + " lain. Silahkan menunggu atau pilih komputer lain!", "Message", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this, "Komputer tersedia.", "Message", JOptionPane.INFORMATION_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(this, "Pilih komputer terlebih dahulu!", "Message", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    if (selectedIndex != -1) {
        ...
        if (customer != null) {
            ...
            int index = computerListModel.indexOf(selectedComputer + " (Digunakan)");
            if (index != -1) {
                computerListModel.set(index, selectedComputer);
            }
            saveQueueData();
            JOptionPane.showMessageDialog(this, "Terima kasih sudah menyewa:)");
        } else {
            JOptionPane.showMessageDialog(this, "Tidak ada pelanggan di komputer ini.");
        }
    } else {
        JOptionPane.showMessageDialog(this, "Pilih komputer terlebih dahulu.");
    }
    }
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("Antrian.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length == 4) {
                    ...
                    customerMap.put(computer, new Customer(name, Integer.parseInt(age)));
                    int index = computerListModel.indexOf(computer);
                    if (index != -1) {
                        computerListModel.set(index, computer + " (Digunakan)");
                    }
    ...
    private void saveQueueData() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("Antrian.txt"))) {
            for (Map.Entry<String, Customer> entry : customerMap.entrySet()) {
                ...
                writer.write(computer + "," + customer.getName() + "," + customer.getAge() + "\n");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
